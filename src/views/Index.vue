      ...
      <b-row style="top:110px ; position:absolute ; height:300px ; width:100%">
        <b-col style="background:none!important">
          <div style="width:65% ; height: 150px ;margin:auto; text-align:center">
            <div class="d-md-none" style="margin:50px"></div><br><br>
            <h2 style="font:48px 'yekan';color:black;font-weight:bolder">خرید و فروش امن بیت‌کوین و ارزهای دیجیتال</h2>
            <h2 style="color:black">به بزرگترین بازار ارز دیجیتال ایران بپیوندید</h2>
          </div>
        </b-col>
      </b-row>
    </div>
        ...
        <table class="table " style="direction:rtl!important">
          <thead>
            <tr style="padding:5px">
              <th  colspan="5" class="col-12"><b-input v-if="infoback2 && infoback && info" style="margin-top:5px;margin-bottom:5px" v-model="searchtext" placeholder="...جستجو"  @input="search()"></b-input></th>
            </tr>
            <tr>
              <th class="col-2 cent">نوع ارز</th>
              <th class="col-3 cent nnmobile"> قیمت دلاری</th>
              <th class="col-2 cent">قیمت خرید(ریال)</th>
              <th class="col-2 cent"> قیمت فروش(ریال)</th>
              <th v-if="false" class="col-3 cent nnmobile">عملیات</th>
            </tr>
          </thead>
          <tbody v-if="info" style="font:20px 'arial';font-weight:bold; max-height:300px" >
            <tr v-if="pp[2] && ('USDT').includes(searchtext.toUpperCase())">
              <td class="col-2 cent"><template class="wmobile"><img src="https://raw.githubusercontent.com/ErikThiart/cryptocurrency-icons/master/64/tether.png" alt=""></template><br>{{pp[2].id}}</td>
              <td class="col-3 cent nnmobile">{{USDTprice}}</td>
              <td class="col-2 cent" style="color:green"><router-link :to="'/buy/' + pp[2].id" class="btn btn-success" style="width: 100% ; color:green!important ; background:none">خرید فوری <hr style="margin:5px ; padding:0 ; border-color:green"> {{(USDTprice * rialprice ).toFixed(2)}}</router-link > </td>
              <td class="col-2 cent" style="color:red"><router-link :to="'/sell/' + pp[2].id"  class="btn btn-danger" style="width: 100% ; color:red!important ; background:none">فروش فوری <hr style="margin:5px ; padding:0 ; border-color:red"> {{(USDTprice * rialprice * 0.994).toFixed(2) }}</router-link > </td>
              <th v-if="false" class="col-4 cent nnmobile"><router-link :to="'/sell/' + pp[2].id" class="btn btn-dark" style="margin:3px"> فروش فوری</router-link> <router-link :to="'/buy/' + pp[2].id" class="btn btn-dark" style="margin:3px">خرید فروی</router-link></th>
            </tr>
            <tr v-for="(item,name,idx) in info" v-bind:key="idx+10000">
              <td v-if="(name.replace('USDT' , '') === 'BTC' | name.replace('USDT' , '') === 'ETH' | name.replace('USDT' , '') === 'TRX' | name.replace('USDT' , '') === 'DOGE' | name.replace('USDT' , '') === 'SHIB' | name.replace('USDT' , '') === 'BNB' | name.replace('USDT' , '') === 'BCH' | name.replace('USDT' , '') === 'XRP')" class="col-2 cent"><template class="wmobile"> <img style="width:64px" :src="`/icons/color/${name.replace('USDT' , '').toLowerCase()}.svg`"  alt=""><img style="width:64px" :src="`/icons/color/${name.replace('USDT' , '').toLowerCase()}.png`"  alt=""></template><br>{{name.replace('USDT' , '')}}</td>
              <td v-if="(name.replace('USDT' , '') === 'BTC' | name.replace('USDT' , '') === 'ETH' | name.replace('USDT' , '') === 'TRX' | name.replace('USDT' , '') === 'DOGE' | name.replace('USDT' , '') === 'SHIB' | name.replace('USDT' , '') === 'BNB' | name.replace('USDT' , '') === 'BCH' | name.replace('USDT' , '') === 'XRP')" class="col-3 cent nnmobile"  >{{item.buy}}</td>
              <td v-if="(name.replace('USDT' , '') === 'BTC' | name.replace('USDT' , '') === 'ETH' | name.replace('USDT' , '') === 'TRX' | name.replace('USDT' , '') === 'DOGE' | name.replace('USDT' , '') === 'SHIB' | name.replace('USDT' , '') === 'BNB' | name.replace('USDT' , '') === 'BCH' | name.replace('USDT' , '') === 'XRP')" class="col-2 cent" style="color:green"><router-link :to="'/buy/' + name.replace('USDT' , '')"  class="btn btn-success" style="width: 100% ; color:green!important ; background:none">خرید فوری <hr style="margin:5px ; padding:0 ; border-color:green"> {{(item.buy * 1.007 * rialprice).toFixed(2)}}</router-link > </td>
              <td v-if="(name.replace('USDT' , '') === 'BTC' | name.replace('USDT' , '') === 'ETH' | name.replace('USDT' , '') === 'TRX' | name.replace('USDT' , '') === 'DOGE' | name.replace('USDT' , '') === 'SHIB' | name.replace('USDT' , '') === 'BNB' | name.replace('USDT' , '') === 'BCH' | name.replace('USDT' , '') === 'XRP')" class="col-2 cent" style="color:red"><router-link :to="'/sell/' + name.replace('USDT' , '')"  class="btn btn-danger" style="width: 100% ; color:red!important ; background:none">فروش فوری <hr style="margin:5px ; padding:0 ; border-color:red"> {{(item.buy * 1.001  * rialprice).toFixed(2)}}</router-link > </td>
              <th v-if="false" class="col-3 cent nnmobile"><router-link :to="'/sell/' + name.replace('USDT' , '')" class="btn btn-dark" style="margin:3px"> فروش فوری</router-link> <router-link :to="'/buy/' + name.replace('USDT' , '')" class="btn btn-dark" style="margin:3px">خرید فروی</router-link></th>
            </tr>
            <tr v-for="(item,name,idx) in info" v-bind:key="idx + 'd'">   
              <td v-if="!(name.replace('USDT' , '') === 'BTC' | name.replace('USDT' , '') === 'ETH' | name.replace('USDT' , '') === 'TRX' | name.replace('USDT' , '') === 'DOGE' | name.replace('USDT' , '') === 'SHIB' | name.replace('USDT' , '') === 'BNB' | name.replace('USDT' , '') === 'BCH' | name.replace('USDT' , '') === 'XRP')"  class="col-2 cent"><template> <img style="width:64px" :src="`/icons/color/${name.replace('USDT' , '').toLowerCase()}.svg`"  alt=""><img style="width:64px" :src="`/icons/color/${name.replace('USDT' , '').toLowerCase()}.png`"  alt=""></template><br>{{name.replace('USDT' , '')}}</td>
              <td v-if="!(name.replace('USDT' , '') === 'BTC' | name.replace('USDT' , '') === 'ETH' | name.replace('USDT' , '') === 'TRX' | name.replace('USDT' , '') === 'DOGE' | name.replace('USDT' , '') === 'SHIB' | name.replace('USDT' , '') === 'BNB' | name.replace('USDT' , '') === 'BCH' | name.replace('USDT' , '') === 'XRP')"  class="col-3 cent nnmobile">{{item.buy}}</td>
              <td v-if="!(name.replace('USDT' , '') === 'BTC' | name.replace('USDT' , '') === 'ETH' | name.replace('USDT' , '') === 'TRX' | name.replace('USDT' , '') === 'DOGE' | name.replace('USDT' , '') === 'SHIB' | name.replace('USDT' , '') === 'BNB' | name.replace('USDT' , '') === 'BCH' | name.replace('USDT' , '') === 'XRP')"  class="col-2 cent" style="color:green"><router-link :to="'/buy/' + name.replace('USDT' , '')"  class="btn btn-success" style="width: 100% ; color:green!important ; background:none">خرید فوری <hr style="margin:5px ; padding:0 ; border-color:green"> {{(item.buy * 1.007 * rialprice).toFixed(2)}}</router-link > </td>
              <td v-if="!(name.replace('USDT' , '') === 'BTC' | name.replace('USDT' , '') === 'ETH' | name.replace('USDT' , '') === 'TRX' | name.replace('USDT' , '') === 'DOGE' | name.replace('USDT' , '') === 'SHIB' | name.replace('USDT' , '') === 'BNB' | name.replace('USDT' , '') === 'BCH' | name.replace('USDT' , '') === 'XRP')"  class="col-2 cent" style="color:red"><router-link  :to="'/sell/' + name.replace('USDT' , '')" class="btn btn-danger" style="width: 100% ; color:red!important ; background:none">فروش فوری <hr style="margin:5px ; padding:0 ; border-color:red"> {{(item.buy * 1.001  * rialprice).toFixed(2)}}</router-link > </td>
              <th v-if="false" class="col-3 cent nnmobile"><router-link :to="'/sell/' + name.replace('USDT' , '')" class="btn btn-dark" style="margin:3px"> فروش فوری</router-link> <router-link :to="'/buy/' + name.replace('USDT' , '')" class="btn btn-dark" style="margin:3px">خرید فروی</router-link></th>
          ...
          <tfoot>
            <tr>
                <th colspan="5" class="col-12 cent d-md-none"><router-link to="/dashboard" class="btn btn-dark">خرید | فروش</router-link></th>
            </tr>
          </tfoot>
        </table>
        </div>
