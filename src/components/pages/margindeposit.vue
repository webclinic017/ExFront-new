<template>
  <div>
    ...
    <h4>انتقال تتر به حساب اصلی معاملات</h4>
      ...
      <fieldset class="demo-vertical-spacing-sm">
        ...
        <b-btn @click="submit()" style="float:left" variant="dark">انتقال به حساب معاملات</b-btn>
      </fieldset>
    </b-card><br><br>


    <h4>انتقال بین حساب اصلی و حساب های مرجین</h4>
    <b-card  class="mb-4 arscard">
    ...
    <fieldset class="demo-vertical-spacing-sm tabsss negative">
        <b-form-group>
          <p style="margin-right:20%">موجودی : <button @click="samount1()" style="padding:2px 30px;" class="btn btn-light">{{balance}}</button></p>
        ...
        <div style="clear:both"></div><br>
        <b-btn @click="submitform1()" style="float:left" variant="dark">انتقال به حساب مرجین</b-btn>
      </fieldset>
      ...
      <fieldset class="demo-vertical-spacing-sm tabsss positive" style="display:none">
        <b-form-group>
          <p style="margin-right:20%">موجودی : <button @click="samount2()" style="padding:2px 30px;" class="btn btn-light">{{coinbalance}}</button></p>
          <label style="width:20%" >از</label>
  ...
  methods: {
    ...
    async getbalance () {
      ...
      await axios
        .get('/cp_mg_main')
        .then(response => {
          this.balance = response.data[this.sym].available
        })
    },
    ...
    samount1(){
      this.amount1 = this.balance
    },
    samount2(){
      this.amount2 = this.coinbalance
    },
    tabact(id) {
