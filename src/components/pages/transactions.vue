<template>
  <div>
          ...
          <div class="col-1 font-weight-bold"></div>
            <div class="d-none d-md-block col-10 text-muted">
              <div class="row no-gutters align-items-center">
                <div class="col-4 ">زمان</div>
                <div class="col-4 ">نوع</div>
                <div class="col-4 cent">مقدار </div>
              </div>
            </div>
        </b-card-header>
              ...
              <div class="row no-gutters align-items-center">
                ...
                <div class="d-none d-md-block col-10">
                  ...
                  <div v-for="transaction in transactions" v-bind:key="transaction" class="row no-gutters align-items-center">
                    ...
                    <div class="col-4 " v-if="transaction.act === 1" style="color:green">{{String(transaction.act).replace('1', 'واریز')}}</div>
                    <div class="col-4 " v-if="transaction.act === 2" style="color:red">{{String(transaction.act).replace('2', 'برداشت')}}</div>
                    <div class="col-4 cent">{{transaction.amount}}</div>
      ...
      <b-card  class="d-md-none ">
            <div v-if="transactions">
              <div v-for="transaction in transactions" v-bind:key="transaction" style="min-height:150px" class="row no-gutters align-items-center">
                <div style="width:45% ;float:right">
              <h2  style=" height:50px ">زمان</h2>
              <h2  style=" height:50px ">نوع</h2>
              <h2  style=" height:50px ">مقدار </h2>
              </div>
              <div style="width:45% ;float:left">
              <h2  style=" height:50px ; text-align:left">{{transaction.date}}</h2>
              <h2  style=" height:50px ; text-align:left ; color:green" v-if="transaction.act === 1" >{{String(transaction.act).replace('1', 'واریز')}}</h2>
              <h2  style=" height:50px ; text-align:left ; color:red" v-if="transaction.act === 2" >{{String(transaction.act).replace('2', 'برداشت')}}</h2>
              <h2  style=" height:50px ; text-align:left">{{transaction.amount}}</h2>
              </div>
                  </div>
            <div v-if="!transactions" class="cent">
              <h3>تراکنشی پیدا نشد</h3>
