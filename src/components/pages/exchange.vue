        ...
        {{sym}}
         ...
         <input @click="listshow" @input="search()" type="text" class="form-control" placeholder="search ..." v-model="searchtxt" style="border-radius: 5px 5px 0 0; border-color:lightgrey!important">
          <div class="list" style="height:150px; overflow-x:hidden ;overflow-y:scroll ; border: solid lightgrey .2px ; border-radius: 0 0 5px 5px" id="my-list-id">
            <button class="d-none d-md-block curbtn" onMouseOver="if(this.style.background === '#ffffff'){this.style.background='#efefef'}" onMouseOut="if(this.style.background === '#efefef'){this.style.background='#ffffff'}" v-for="(value , key) in leverage" v-bind:key="key" :id="key" type="button"  style="height:50px; width: 33.33% ; float:left;text-align:left!important; background: #ffffff ;border-style: none; border-bottom: solid .2px lightgrey ;border-shadow:none margin:0 ; font: 15px 'arial';padding-left:10%; border-right:solid .2px lightgrey  "  @click="buttonchange(key)"  selected >{{key}}&emsp;<template style="margin:9px"><cryptoicon :symbol="key" size="32"  /></template>   </button>
            <button class="d-md-none curbtn" onMouseOver="if(this.style.background === '#ffffff'){this.style.background='#efefef'}" onMouseOut="if(this.style.background === '#efefef'){this.style.background='#ffffff'}" v-for="(value , key) in leverage" v-bind:key="'n' + key" :id="key"  type="button"  style="height:50px; width: 100% ; background: #ffffff ;border-style: none; border-bottom: solid .2px lightgrey ;border-shadow:none margin:0 ; font: 15px 'arial';padding-left:10%"  @click="buttonchange(key)"  selected >{{key}}&emsp;<template style="margin:9px"><cryptoicon :symbol="key" size="32"  /></template>   </button>
          ...
          <div class="listtwo" style="height:150px; overflow-x:hidden ;overflow-y:scroll ; border: solid lightgrey .2px ; border-radius: 0 0 5px 5px" id="my-list-id">
            <button class="d-none d-md-block curbtntwo" onMouseOver="if(this.style.background === '#ffffff'){this.style.background='#efefef'}" onMouseOut="if(this.style.background === '#efefef'){this.style.background='#ffffff'}" v-for="(value , key) in leverage2" v-bind:key="key + 'two'" :id="key + 'two'" type="button"  style="height:50px; width: 33.33% ; float:left;text-align:left!important; background: #ffffff ;border-style: none; border-bottom: solid .2px lightgrey ;border-shadow:none margin:0 ; font: 15px 'arial';padding-left:10%; border-right:solid .2px lightgrey  "  @click="buttonchange2(key)"  selected >{{key}}&emsp;<template style="margin:9px"><cryptoicon :symbol="key" size="32"  /></template>   </button>
            <button class="d-md-none curbtn" onMouseOver="if(this.style.background === '#ffffff'){this.style.background='#efefef'}" onMouseOut="if(this.style.background === '#efefef'){this.style.background='#ffffff'}" v-for="(value , key) in leverage2" v-bind:key="'n' + key + 'two'" :id="key + 'two'"  type="button"  style="height:50px; width: 100% ; background: #ffffff ;border-style: none; border-bottom: solid .2px lightgrey ;border-shadow:none margin:0 ; font: 15px 'arial';padding-left:10%"  @click="buttonchange2(key)"  selected >{{key}}&emsp;<template style="margin:9px"><cryptoicon :symbol="key" size="32"  /></template>   </button>
            <h2 v-if="!leverage" style="font-family:'arial'; text-align:center">موردی یافت نشد</h2>
          </div><br><br>
...
export default {
  ...
  methods: {
    ...
    buttonchange(key) {
      ...
      document.querySelector(`#${key}`).style.background = 'grey'
      this.search2()
      this.getrial()
    },
    buttonchange2(key) {
      ...
      document.querySelector(`#${key}two`).style.background = 'grey'
      this.search()
    },
    search(){
      if(this.searchtxt){
