  ...
  <div>
    ...
    <div class="py-1 mb-2">
      <b-input-group>
        ...
        <b-btn slot="append" variant="dark"><i class="ion ion-ios-search"></i>&nbsp; Search</b-btn>
      </b-input-group>
    </div>
      ...
      <div class="row">
        ...
        <div v-for="user in searchData.people" :key="user.username" class="col-md-6 col-xl-4">
          <b-card class="card-condenced mb-4" body-class="media align-items-center">
            <img :src="`${publicUrl}img/avatars/${user.avatar}`" alt="" class="ui-w-40 rounded-circle">
            <div class="media-body ml-3">
              <a href="javascript:void(0)" class="text-body font-weight-semibold mb-2">{{user.name}}</a>
              <div class="text-muted small">@{{user.username}}</div>
            </div>
            <b-btn v-if="!user.following" variant="outline-dark rounded-pill md-btn-flat" size="sm" class="d-block">Follow</b-btn>
            <b-btn v-if="user.following" variant="success rounded-pill md-btn-flat" size="sm" class="d-block">Following</b-btn>
          </b-card>
        </div>
    ...
    <div v-if="curTab === 'images'">
      ...
      <div class="form-row">
        ...
        <div v-for="image in searchData.images" :key="image.path" class="col-sm-6 col-xl-4 mb-2">
          <a href="javascript:void(0)" class="ui-rect-60 ui-bg-cover ui-bg-overlay-container text-white" :style="{'background-image': `url('${publicUrl}${image.path}')`}">
    ...
    <div v-if="curTab === 'videos'">
      ...
      <div class="row">
        ...
        <div v-for="video in searchData.videos" :key="video.preview" class="col-sm-6 col-md-4 col-xl-3 mb-4">
          <a href="javascript:void(0)" class="ui-rect-60 ui-bg-cover text-white" :style="{'background-image': `url('${publicUrl}${video.preview}')`}">
          ...
          <a href="javascript:void(0)" class="d-block text-body mt-2">{{video.title}}</a>
          <div>
