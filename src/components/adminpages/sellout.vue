        ...
        <div v-for="section in requests" :key="section.title">
          <b-card-body class="py-3 wallets">
            ...
            <div class="row no-gutters align-items-center" style="margin:0;margin-top:15px">
              ...
              <div class="col-8 cent"> <input class="form-control" type="text" readonly :value="section.hash" style="width:90%"></div>
...
export default {
  ...
  methods: {
    ...
    async accept (id) {
      ...
      await axios
        .post('adminpanel/sellout' , {id: id , act:'accept' , rramount: amount})
        .then(response => {
          this.getc()
        })
    },
    async reject (id) {
      await axios
        .post('adminpanel/sellout' , {id: id , act:'reject'})
        .then(response => {
          this.getc()
        })
    },
