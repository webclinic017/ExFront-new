  ...
  <div>
      ...
      <b-card no-body class="col-12 d-none d-md-block">
        ...
        <b-card-header class="row no-gutters align-items-center">
          ...
          <div class="col-2 cent">کد هش</div>
      ...
      <div  class="d-md-none transm">
            <div v-if="requests">
              ...
              <h4  style=" height:50px ; text-align:left">{{transaction.hash}}</h4>
              <h5  style=" height:50px ; text-align:left">{{transaction.get_age}}</h5><br>
              </div>
              <div class="col-12 cent"><button @click="accept(transaction.id)" class="btn btn-success" style="width:100%; float:left">تایید</button><button @click="reject(transaction.id)" class="btn btn-danger " style="width:100%; float:left">رد</button></div>
                  </b-card><br>
...
export default {
  ...
  methods: {
    ...
    async accept (id) {
      await axios
        .post('adminpanel/cp_deposit' , {id: id , act:'accept'})
        .then(response => {
          this.getc()
        })
    },
    async reject (id) {
      await axios
        .post('adminpanel/cp_deposit' , {id: id , act:'reject'})
        .then(response => {
          this.getc()
        })
    },
