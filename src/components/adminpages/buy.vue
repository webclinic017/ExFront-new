        ...
        <div v-for="section in requests" :key="section.title">
          <b-card-body v-if="section" class="py-3 wallets">
            ...
            <div class="row no-gutters align-items-center">
              ...
              <div class="col-2 cent"><form @submit="accept(section.id)"><input required class="form-control" min="100000" :id="section.id" style="width:100%; float:left" type="number" placeholder=" قیمت تمام شده"><button type="submit" class="btn btn-success" style="width:100%; float:left">تایید</button></form><button @click="reject(section.id)" class="btn btn-danger " style="width:100%; float:left">رد</button></div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async accept (id) {
      ...
      await axios
        .post('adminpanel/buy' , {id: id , act:'accept' , rramount: amount})
        .then(response => {
          this.getc()
        })
    },
    ...
    async reject (id) {
      await axios
        .post('adminpanel/buy' , {id: id , act:'reject'})
        .then(response => {
          this.getc()
        })
    },
