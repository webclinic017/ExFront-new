  ...
  <div>
      ...
      <b-card no-body class="col-12 d-none d-md-block">
        ...
        <b-card-header class="row no-gutters align-items-center">
          ...
          <div class="col-2 cent">نوع ارز پرداختی</div>
          <div class="col-1 cent">پرداختی</div>
          <div class="col-2 cent">نوع ارز دریافتی</div>
          <div class="col-1 cent">دریافتی</div>
      ...
      <div  class="d-md-none transm">
            <div v-if="requests">
              ...
              <h6  style=" height:50px ">نوع ارز پرداختی</h6>
              <h6  style=" height:50px ">پرداختی</h6>
              <h6  style=" height:50px ">نوع ارز دریافتی</h6>
              <h6  style=" height:50px ">دریافتی </h6>
              ...
              <h4  style=" height:50px ; text-align:left">{{transaction.currency2}}</h4>
              <h4  style=" height:50px ; text-align:left">{{transaction.camount2}}</h4>
              <h5  style=" height:50px ; text-align:left">{{transaction.get_age}}</h5><br>
              </div>
              <div class="col-12 cent"><form @submit="maccept(transaction.id)"><input required class="form-control" min="100000" :id="'m' +  transaction.id" style="width:100%; float:left" type="number" placeholder=" قیمت تمام شده"><button type="submit" class="btn btn-success" style="width:100%; float:left">تایید</button></form><button @click="reject(transaction.id)" class="btn btn-danger " style="width:100%; float:left">رد</button></div>
                  </b-card><br>
...
export default {
  ...
  methods: {
    ...
    async accept (id) {
      await axios
        .post('adminpanel/exchange' , {id: id , act:'accept'})
        .then(response => {
          this.getc()
        })
    },
    async maccept (id) {
      ...
      await axios
        .post('adminpanel/exchange' , {id: id , act:'accept' , rramount: amount})
        .then(response => {
          this.getc()
        })
    },
    async reject (id) {
      await axios
        .post('adminpanel/exchange' , {id: id , act:'reject'})
        .then(response => {
          this.getc()
        })
    },
